...
-- Declare the form. The type signature is a bit intimidating,
-- but here's the overview:
--
-- * The Html parameter is used for encoding some extra information.
-- See the discussion regarding runFormGet and runFormPost below for
-- further explanation.
--
-- * We have our Handler as the inner monad, which indicates which
-- site this is running in.
--
...
--
-- Note that the scaffolded site provides a convenient Form type
-- synonym, so that our signature could be written as:
--
...
-- The POST handler processes the form. If it is successful,
-- it displays the parsed person. Otherwise, it displays the form
-- again with error messages.
